...
- Anthropic は Cloudflare Worker 経由（`cares-relay.agewaller.workers.dev` を使用。ブラウザから直接アクセスする唯一の外向きエンドポイント）
- `ai.cares.advisers.jp` カスタムドメインは現在停止中（`advisers.jp` ゾーンが Cloudflare から削除されているため）。復旧した場合は `wrangler.relay.jsonc` の `routes` を戻し `CLAUDE.md` を更新すること。それまでは `cares-relay.agewaller.workers.dev` を使う
