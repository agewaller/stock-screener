...
- **デフォルト**: `claude-opus-4-8`（最新・最強、課金OK）
- 選択肢: Opus 4.8 / Opus 4.7 / Opus 4.6 / Sonnet 4.6 / Haiku 4.5 / GPT-4o / Gemini 2.5 Pro
- Anthropic は Cloudflare Worker 経由（固定エンドポイント `https://ai.cares.advisers.jp`）
- `ai.cares.advisers.jp` は `stock-screener` Worker のカスタムドメイン（`wrangler.jsonc` の `routes` で `custom_domain: true` 登録）。`*.workers.dev` のアカウントレベル無効化と Cloudflare Access を回避するため、ブラウザは必ずこの独自ドメインを使う。`cares-relay.agewaller.workers.dev` は rollback / diagnostics 用の旧経路としてのみ残存
...
- プロバイダ API 鍵（Anthropic/OpenAI/Google 等）は **サーバ（Cloudflare Worker の
  ...
  AI 呼び出しは必ず `ai.cares.advisers.jp` → `stock-screener` Worker 経由（ブラウザは鍵を送らない・受け取らない）。
- ブラウザの単一 AI エンドポイントはコードに固定（`AIEngine.PROXY_URL`）。
  **localStorage の proxy 上書きは廃止**。古い壊れた
...
### デプロイ時のサーバ設定（cares-ai-proxy Worker）

- Secret: `ANTHROPIC_API_KEY`（フォールバック）, `ADMIN_WRITE_TOKEN`（緊急用）
- Var: `FIREBASE_PROJECT_ID`（ID トークン検証用）, `ALLOWED_ORIGINS`（省略可）
- Custom Domain: `ai.cares.advisers.jp`（`stock-screener` Worker に直接アタッチ）
- KV: `RESEARCH_KV`（research context と `admin:key:*` を共用。namespace 名は
  `stock-screener-research`。`deploy-worker.yml` がIDを解決して `wrangler.jsonc` に注入）
