...
- `ai.cares.advisers.jp` は cares-relay Worker のカスタムドメイン（`wrangler.relay.jsonc` の `routes` で `custom_domain: true` 登録）。`*.workers.dev` のアカウントレベル無効化を回避するため、必ずこの独自ドメインを使う。`cares-relay.agewaller.workers.dev` / `cares-ai-proxy.agewaller.workers.dev` への直接アクセスはフォールバックとしてのみ残存
