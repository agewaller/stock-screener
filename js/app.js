...
${joined.substring(0, 8000)}`;
  ...
  async restoreFromBackup(backupId) {
    ...
    if (out) {
      ...
      out.innerHTML = `<div style="background:var(--warning-bg,#fef3c7);border:1px solid var(--warning,#f59e0b);border-radius:var(--radius,8px);padding:12px;margin-top:8px">
        <p style="margin:0 0 10px;font-size:14px">「${safeId}」から復元しますか？<br><small>本日のデータも残ります（マージ動作）</small></p>
        <button onclick="app._executeRestoreFromBackup('${safeId}')" style="background:var(--danger,#dc2626);color:#fff;border:none;border-radius:var(--radius-sm,4px);padding:8px 16px;font-size:14px;cursor:pointer;margin-right:8px">復元する</button>
        <button onclick="document.getElementById('backup-list-result').innerHTML=''" style="background:var(--surface-2,#f3f4f6);border:1px solid var(--border,#e5e7eb);border-radius:var(--radius-sm,4px);padding:8px 16px;font-size:14px;cursor:pointer">キャンセル</button>
      </div>`;
    }
  }
