  ...
  dashQuickFile(event) {
    ...
    Array.from(files).forEach(async (file) => {
      ...
      reader.onload = (ev) => {
        ...
        const analysis = this.generateDeepAnalysis(textForAnalysis.join('\n'));
        store.set('latestFeedback', analysis);
      ...
      reader.onload = (ev) => {
        ...
        if (resultEl) {
          ...
          resultEl.innerHTML = `<div class="card" style="border-left:4px solid var(--accent)">
            ${fb.sections.map(s => `<div style="padding:10px 14px;border-bottom:1px solid var(--border)"><div style="font-size:12px;font-weight:600;margin-bottom:4px">${s.icon} ${s.title}</div><div style="font-size:12px;color:var(--text-secondary);line-height:1.7;white-space:pre-wrap">${s.content}</div></div>`).join('')}
          </div>`;
        }
      };
      reader.readAsDataURL(file);
