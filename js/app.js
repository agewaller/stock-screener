...
${titles}`;
  ...
  generateQuickInsight(text) {
    ...
    if (/alt|ast|γgtp|肝機能|肝硬変|腹水|黄疸|脂肪肝|mash/.test(lower)) return '肝機能記録。ALT/AST・アルブミン・血小板の推移が線維化評価の基本指標です。';
    if (/cipn|しびれ|しびれ感|末梢神経|化学療法|抗がん剤|パクリタキセル/.test(lower)) return '化学療法副作用の記録。CIPNは早期報告で投与量調整・デュロキセチン追加が可能です。';
    if (/血圧|収縮期|拡張期|高血圧/.test(lower)) return '血圧記録。朝・就寝前の家庭血圧が心血管リスク管理の基本です。';
    if (/血糖|hba1c|インスリン|糖尿病/.test(lower)) return '血糖管理の記録。食後2時間血糖とHbA1cの推移を追跡中。';
    if (/発作|てんかん|痙攣/.test(lower)) return '発作の記録。発作日時・持続時間・誘因の記録が薬の調整に直結します。';
    if (/心房細動|afib|不整脈|動悸/.test(lower)) return '不整脈の記録。動悸の発生時刻・持続時間・活動状況を記録すると診断に役立ちます。';
    if (/関節|腫れ|こわばり|リウマチ/.test(lower)) return '関節症状の記録。DAS28などの活動性スコアとの相関を追跡中。';
