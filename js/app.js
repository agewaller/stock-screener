...
URL/連絡先：（あれば）`;
  ...
  translateUrl(url) {
    ...
    // Detect site language from URL domain
    const siteLangMap = {
      'ja': ['.co.jp', '.jp', 'peatix.com', 'byoinnavi.jp', 'caloo.jp', 'nanbyou.or.jp', 'note.com', 'connpass.com', 'stoacka.jp'],
      'en': ['.com', '.org', '.io', '.dev', 'pubmed.ncbi', 'clinicaltrials.gov', 'iherb.com', 'eventbrite.com', 'meetup.com', 'reddit.com', 'github.com'],
    ...
    // Check if site matches user's language
    const lowerUrl = url.toLowerCase();
    const userDomains = siteLangMap[lang] || [];
    if (userDomains.some(d => lowerUrl.includes(d))) return url; // Same language, no translation

    // Check if site is English (many international sites)
    if (lang === 'en') return url;

    // For other combinations, translate
