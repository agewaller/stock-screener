...
(function () {
  ...
  var CATALOGS = {
    ...
    low_back_pain: {
      title: '📚 慢性腰痛・腰椎疾患の管理に役立つリソース',
      items: [
        { kind: 'book', label: '『腰痛は動かして治す — 最新保存療法・運動療法ガイド』', q: '腰痛 運動療法 保存療法 治し方 本' },
        { kind: 'device', label: 'TENS低周波治療器（家庭用・腰痛の疼痛緩和）', q: 'TENS 低周波治療器 腰痛 家庭用 疼痛 緩和' },
        { kind: 'device', label: 'ランバーサポート（腰椎クッション・デスクワーク用）', q: 'ランバーサポート 腰椎 クッション 腰痛 オフィス チェア' },
        { kind: 'supplement', label: 'ビタミンD3（神経障害性疼痛・骨密度維持）', q: 'ビタミンD3 2000IU 骨密度 腰痛 神経 サプリ' },
        { kind: 'device', label: 'ホットパック・遠赤外線カイロ（温熱療法・腰部加温）', q: 'ホットパック 温熱 遠赤外線 腰部 腰痛 慢性' }
      ]
    },
    glaucoma: {
      title: '📚 緑内障の管理に役立つリソース',
      items: [
        { kind: 'book', label: '『緑内障と向き合う — 視野を守る点眼継続のすべて』', q: '緑内障 視野 点眼 治療 本' },
        { kind: 'device', label: '点眼補助器具（点眼が難しい方のサポートグッズ）', q: '点眼補助 目薬 器具 高齢者 緑内障' },
        { kind: 'book', label: '『正常眼圧緑内障を知る — 失明予防の最前線』', q: '正常眼圧緑内障 失明予防 眼圧 視野 本' },
        { kind: 'supplement', label: 'ルテイン+ゼアキサンチン（視神経・網膜の保護サポート）', q: 'ルテイン ゼアキサンチン 目 視神経 サプリ 緑内障' },
        { kind: 'device', label: 'UVカット眼鏡（視神経への紫外線保護）', q: 'UVカット サングラス 眼鏡 紫外線 視神経 保護 緑内障' }
      ]
    }
  };
