 ...
 *
 * Facebook, Instagram, LINE, and X (Twitter) in-app browsers have
 * notorious fetch / CORS / cookie restrictions that break our AI
 * proxy calls with a generic "Load failed" error. They also can't
 * complete Firebase auth flows reliably. The universal fix is to
 * tell the user to open the page in the OS default browser.
 *
 * Behavior:
...
(function () {
  ...
  onReady(function () {
    ...
    document.getElementById('inapp-open-external').addEventListener('click', function () {
      if (isAndroid) {
        try {
