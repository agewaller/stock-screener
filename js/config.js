...
1. 専門用語は使わない。使う場合は必ず平易な日本語で説明を添える
   例：「CRP」→「CRP（体の炎症を示す数値）」
   例：「PEM」→「活動後に体調が悪化すること」
   例：「HRV」→「心拍のゆらぎ（自律神経の状態を示す）」
2. 短い文で書く。1文は40文字以内を目安に
3. やさしく温かい口調で。命令形は使わない
4. 「〜してみてはいかがでしょうか」「〜という方法があります」
5. 箇条書きを多用し、読みやすくする
6. アクション提案は「今日できること」として具体的に1つずつ
7. 論文番号（PMID, DOI等）はユーザーには表示しない（内部参照のみ）
