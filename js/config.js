...
### 出力形式（必ず JSON のみ、前後の説明なし）
{
  ...
  GUEST_SAMPLES: {
    ...
    migraine: [
      '左側の拍動性頭痛 7/10。光が眩しく、吐き気もある。トリプタンを飲んで横になっている。',
      '昨日の気圧低下（台風接近）で前駆症状が出た。首のこわばりと視野のギラギラ（閃輝暗点）。',
      '今月4回目の発作。月経前後に集中している。ピルとの関係を医師に相談したい。'
    ],
    adhd: [
      '今日も仕事の締め切りを2つ忘れた。頭では分かっているのに行動に移せない。自己嫌悪が強い。',
      'コンサータ服薬日は午前中の集中力が全然違う。タスクを15分ブロックに分けると達成感がある。',
      '会議中に話が飛び散ってしまい、皆についていくのが大変。でも今日は議事録を最後まで書けた。'
    ],
    bipolar: [
      '先週の躁状態から一転して今日はどん底。布団から出られない。気分の落差が激しすぎて疲れた。',
      '気分安定薬（リーマス）を飲み始めて2週間。波は少し穏やかになってきた気がする。',
      '今朝の気分は普通（5/10）。こういう「普通」が続いてほしい。睡眠 7 時間確保できた。'
    ],
    endometriosis: [
      '生理3日目。痛みが10/10で職場のトイレで動けなくなった。鎮痛剤が効かない。',
      'ジエノゲスト（ビジュアル）開始2ヶ月。生理量は減ったが不正出血が続いている。',
      '腹腔鏡手術から6ヶ月。痛みは7→3/10に改善。ただ疲労感と腸の不快感は残っている。'
    ],
    chronic_pain: [
      '今日の痛み 8/10。昨日より悪い。原因不明の持続痛で何もできない一日だった。',
      'ペインクリニックでブロック注射。3時間は楽になったが夕方からまた戻ってきた。',
      '気分と痛みの連動が気になる。ストレスがかかると確実に痛みが増す。記録して医師に見せたい。'
    ],
    gad: [
      '朝から心配事が止まらない。仕事・お金・健康と次々と不安が浮かんでは消えない。',
      'SSRI開始4週目。以前よりざわざわ感が少なくなった気がする。身体症状（動悸・胃痛）も軽減。',
      '今日は不安レベル 4/10（普段は7〜8）。呼吸法が少しずつ効いてきた。'
    ],
    ptsd: [
      '夜中にまたフラッシュバック。汗びっしょりで目が覚めた。眠れなかった。',
      'EMDR3回目。セッション後は疲労感が強いが、少し処理が進んでいる感覚。',
      '今日はトリガー（バスの中の人混み）を乗り越えられた。小さな前進。'
    ],
    cptsd: [
      '感情の調整が難しい。些細なことで爆発してしまい、後悔して落ち込む繰り返し。',
      '身体にずっと緊張が残っている感じ。安全な場所にいるのに心拍が高い（90bpm）。',
      '今日はグラウンディング（足裏に意識を向ける）が効いた。5分だけど落ち着けた。'
    ],
    ra: [
      '朝のこわばり 90 分。両手の指が腫れていて、ペットボトルのキャップも開けられない。',
      'MTX 服薬日。吐き気が強い。葉酸を翌日に飲むようにしたら少しマシになった。',
      '今週は CRP が下がってきた（採血結果）。疲労感も 7→5 に改善。治療が効いてきた。'
    ],
    ms: [
      '今日は左足がしびれている。視野がぼやける症状も少し出た。再発かもしれない。',
      'コパキソン（グラチラマー）注射6ヶ月継続。今年の再発はまだゼロ。効いている気がする。',
      '疲労感が強い。ちょっと歩くだけで足が重くなる。リハビリと休息のバランスを探っている。'
    ],
    pcos: [
      '3ヶ月生理が来ない。体重が増えやすく、ニキビも悩み。ピルを始めることにした。',
      'メトホルミン開始2週間。胃腸症状は少し落ち着いてきた。体重は 0.5kg 減。',
      '超音波で卵胞が多数確認された。インスリン抵抗性の改善のため食事と運動を継続している。'
    ],
    thyroid_hypo: [
      'チラージン 50μg 内服中。まだ疲労感と冷えが抜けない。TSH は 4.2（正常範囲内と言われた）。',
      '髪が抜けやすく、体重が 3kg 増えた。便秘と冷えもひどい。次回の採血で用量を相談したい。',
      '薬を増量して 1 ヶ月。だいぶ楽になってきた。朝の浮腫も改善。TSH 1.8 に下がった。'
    ],
    thyroid_hyper: [
      '動悸がひどく、じっとしていても心拍 100 以上。汗をよくかく。メルカゾール開始。',
      'バセドウ病と診断されて 3 ヶ月。甲状腺腫が少し小さくなってきた。FT4 も正常範囲に入った。',
      '今日の心拍 78bpm（以前は 110）。体重が戻ってきた。だいぶ安定してきた。'
    ],
    default: [
