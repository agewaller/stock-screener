...
App.prototype.render_dashboard = function() {
  ...
  ${(() => {
    ...
    const hints = {
      ...
      low_back_pain: ['腰痛NRS(0-10)', 'しびれの部位・強さ', '歩行距離(m)', 'プレガバリン服薬確認'],
      glaucoma: ['点眼確認（時刻）', '眼圧（受診時）', '視力・見え方の変化', '充血・刺激感'],
      ...
      low_back_pain: ['腰痛NRS(0-10)・下肢しびれの日次記録', '歩行可能距離・間欠性跛行の閾値記録', 'プレガバリン・NSAIDs・筋弛緩薬の服薬確認と副作用', '神経ブロック・手術後の疼痛変化の追跡'],
      glaucoma: ['毎日の点眼確認（時刻・左右・回数）', '受診時の眼圧値（右・左）の記録', '視野MD値・OCT-RNFL厚の定期記録', 'β遮断薬使用時の脈拍・呼吸症状確認'],
    };
    const items = new Set();
