...
App.prototype.render_dashboard = function() {
  ...
  ${(() => {
    ...
    const hints = {
      ...
      ptsd: ['気分', '睡眠', 'フラッシュバック'],
      cptsd: ['フラッシュバック頻度(0-10)', '感情の強さ・種類', '解離エピソードの有無', 'グラウンディング実施'],
      adhd: ['集中力', '睡眠', 'タスク達成'],
      ...
      als: ['ALSFRS-Rスコア（週次）', '構音障害・嚥下の変化', '呼吸機能（SpO₂・努力肺活量）', '服薬確認（リルゾール・エダラボン）'],
      ...
      obesity: ['体重・BMI・腹囲の日次記録', 'GLP-1薬服薬確認・副作用（悪心・下痢）', '食事カロリー・食欲変化の評価', '運動時間・歩数の記録'],
      stroke: ['血圧（朝・夕の家庭血圧）', '抗凝固薬・抗血小板薬服薬確認', '神経症状の変化（麻痺・言語・嚥下）', 'リハビリ内容・時間・進捗'],
      dissociative: ['解離エピソードの日時・持続時間・強度', 'トリガー（引き金）の記録', 'グラウンディング技法の実施と効果', '感情・内的状態・現実感（0-10）'],
      pulmonary_fibrosis: ['SpO₂（安静時・労作時・夜間）', '息切れ・mMRCスコアの日次記録', 'ピルフェニドン/ニンテダニブ服薬確認・副作用', '6分間歩行テスト（距離・最低SpO₂）'],
    };
    const items = new Set();
