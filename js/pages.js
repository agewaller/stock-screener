...
App.prototype.render_dashboard = function() {
  ...
  <div style="display:flex;flex-wrap:wrap;gap:4px">${diseaseTagsHtml}</div>`;
    ...
    return `<div style="padding:20px"><p>読み込みエラーが発生しました。</p><p style="font-size:12px;color:var(--text-muted)">${Components.escapeHtml(err?.message || String(err))}</p><button class="btn btn-primary" onclick="store.clearAll();location.reload()">データをリセットして再読み込み</button></div>`;
  }
};
