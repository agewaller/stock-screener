...
App.prototype.render_dashboard = function() {
  ...
  ${(() => {
    ...
    const hints = {
      ...
      gerd: ['胸やけ・呑酸の頻度と強さ(1-10)', '症状発生時刻と最終食事からの時間', 'PPI/タケキャブ服薬確認（食前30分）', '悪化した食品・体位・ストレスの記録'],
      obesity: ['体重（起床後・排尿後）', 'ウエスト周囲（週1回）', '食事内容・カロリー概算', 'GLP-1注射日・副作用（悪心・食欲変化）'],
      als: ['ALSFRS-Rスコア（12項目）月次記録', '体重・嚥下時間・誤嚥頻度', 'SpO2・夜間低酸素・疲労感', 'リルゾール・エダラボン服薬・副作用（肝機能）'],
      metabolic_syndrome: ['腹囲（週1回・起床後）', '体重・BMI・血圧（毎朝）', '空腹時血糖・中性脂肪・HDL（受診時）', '歩数・運動時間・食事内容概算'],
      celiac: ['グルテン摂取有無・混入リスクの確認', '腹痛・下痢・倦怠感・ブレインフォグの程度', 'TTG-IgA・鉄・ビタミンD（受診時）', '外食・旅行でのグルテンフリー対応状況'],
      cptsd: ['トリガー出来事・感情の波（0-10）', '解離・フラッシュバックの頻度と持続時間', '睡眠・悪夢・夜間覚醒の状態', 'グラウンディング技法の使用と効果'],
      pulmonary_fibrosis: ['FVC・SpO2・6分間歩行距離（受診時）', '労作時息切れ・乾性咳嗽の頻度と強さ(1-10)', 'ピルフェニドン/ニンテダニブ服薬・副作用（食欲・下痢）', 'HOT使用時間・酸素流量・夜間SpO2'],
      dysautonomia: ['起床直後の臥位→起立後 2・5・10 分の心拍・血圧', 'HRV（心拍変動）・Oura/Garmin 値', '症状スコア（めまい・動悸・脳霧・倦怠感 0-10）', '塩分・水分摂取量・圧迫ストッキング使用の確認'],
      sibo: ['腹部膨満・ガス・腹痛の強さ（食後30分・2時間）', '今日食べた高FODMAP食品の特定', 'リファキシミン服薬日・便の形状（Bristolスケール）', '排便回数・腹部症状と食事の相関メモ'],
      gastroparesis: ['食後の悪心・嘔吐回数・膨満感（0-10）', '食事内容・脂質量・1回の量と食後症状', 'ドンペリドン/メトクロプラミド服薬タイミング確認', '体重・食事摂取量・水分摂取量の推移'],
      dissociative: ['解離エピソードの頻度・持続時間・トリガー', '離人感/現実感消失の強さ（0-10）', '記憶の空白・状態の切り替わりの確認', 'グラウンディング技法の使用と効果'],
      lyme: ['抗生物質服薬状況・副作用（消化器・光線過敏）', '関節痛・筋肉痛・疲労・認知症状（0-10）', 'マダニ刺咬・遊走性紅斑の有無と記録', '活動量と体調悪化の相関（PEM 様反応の確認）'],
      polymyalgia: ['朝のこわばり持続時間（分）・起床時痛みスコア（0-10）', 'プレドニゾロン用量と CRP/ESR 値（受診時）', 'ステロイド副作用：体重・血糖・骨密度・感染症リスク', 'GCA 合併サイン確認：新しい頭痛・視覚変化・顎跛行'],
      raynauds: ['発作の頻度・持続時間・誘因（気温・水温・ストレス）', '色変化の段階（白・青・赤）と罹患部位（指・趾・耳）', 'カルシウム拮抗薬服薬と発作頻度の変化', '指端潰瘍・新しい皮膚硬化などの続発性サインの確認'],
      tbi: ['頭痛・疲労・認知症状・光/音過敏の強さ（0-10）', '活動量と症状悪化の相関（Berlin プロトコル段階確認）', '睡眠時間・質・昼間の眠気の追跡', '感情変動（易怒性・抑うつ）・PTSD 症状の確認'],
      neuropathy: ['しびれ・灼熱感・疼痛の強さ（0-10）・部位・時間帯', '血糖値・HbA1c（糖尿病合併者）の記録', 'プレガバリン/デュロキセチン服薬と疼痛軽減の効果', '転倒回数・バランス状態・足の傷/変色の確認'],
      nafld: ['体重・BMI・腹囲（週1回）', 'AST・ALT・γGTP・FIB-4（受診時）', '食事カロリー・糖質・果糖（清涼飲料）摂取量', '運動時間・歩数（週150分目標の達成率）'],
      hsd: ['関節痛・筋肉痛の部位と強さ（0-10）・脱臼/亜脱臼回数', 'POTS 症状（起立後の心拍・めまい）・疲労レベル', 'リハビリ内容（種目・回数）と翌日の症状変化', 'MCAS 発作・アレルギー反応のトリガーと頻度'],
      dvt: ['抗凝固薬（DOAC/ワーファリン）服薬時刻・出血サインの確認', 'PT-INR・D ダイマー（受診時）', '下肢の腫脹・疼痛・皮膚色の変化を毎日確認', '出血リスク：歯茎・血尿・黒色便・異常な打撲'],
      adrenal: ['ヒドロコルチゾン服薬用量・タイミング（増量日の記録含む）', '朝の血圧（臥位→起立後）・倦怠感スコア（0-10）', '発熱・嘔吐・感染などシックデイイベントの有無', '血清コルチゾール・電解質・血糖（受診時）のトレンド'],
      ihd: ['胸痛・息切れ・動悸の発生状況（労作時/安静時・持続時間）', '服薬確認：アスピリン・スタチン・β遮断薬・DAPT', '朝の血圧・脈拍・体重の記録', '運動量（歩数・有酸素運動時間）と心臓リハビリの進捗'],
      cancer_survivor: ['疲労・認知症状・しびれの強さ（0-10）の毎日記録', '運動量（歩数・有酸素時間）の継続追跡', '再発不安のレベルと引き金（受診前・体の変化）', '服薬（ホルモン療法・支持療法薬）と副作用を記録'],
      chemo_side: ['吐き気・嘔吐・口内炎・倦怠感のスコア（0-10）を毎日記録', '体温測定（発熱性好中球減少症の早期発見）', '制吐薬・G-CSF・支持療法薬の服薬タイミングと効果', '手足のしびれ・体重変化・出血サインの継続追跡'],
      immunodeficiency: ['感染エピソード（発熱・咳・下痢）の発生日・部位・治療内容', 'IVIG/SCIG 投与日・用量・IgGトラフ値（次回投与前）の記録', '体温（感染早期発見）・倦怠感・活動量の毎日追跡', '予防的抗菌薬の服薬と副作用・消化器症状の確認'],
      mcs: ['化学物質暴露イベント（場所・推定誘因・時刻）の即時記録', '症状の種類（頭痛・倦怠感・認知・気道・皮膚）と強さ（0-10）', '暴露から発症までの時間・回復時間のトレンド', '回避できた日（無症状）と回避失敗日の比較追跡'],
      eating: ['食事内容・摂取量・スキップした食事の記録（罰なく中立に）', '身体症状（めまい・倦怠感・心拍・手足の冷え）と食事摂取量の関係', '気分・食への不安・食後の補償行動衝動のスコア（0-10）', '体重・体温（低体温は低栄養サイン）・服薬（ビタミン・補充剤）'],
      substance: ['飲酒量（ml・標準ドリンク数）または薬物使用の記録', '離脱症状の有無（発汗・振戦・不安・不眠のスコア）', '服薬（ナルトレキソン・バレニクリン等）と渇望レベル（0-10）', 'クリーンな日数・リラプスのトリガー（感情・社会的状況）の記録'],
      allergy: ['症状（目・鼻・皮膚・気道）の強さ（0-10）と誘因を毎日記録', '抗ヒスタミン薬・外用薬・舌下免疫療法の服薬記録と効果', '花粉・食品・ダニ等のアレルゲン暴露と症状の関係を追跡', 'アナフィラキシーエピソードの詳細記録（誘因・症状・対応）'],
      mold: ['症状スコア（疲労・ブレインフォグ・頭痛）と居場所（自宅/職場/外）の関係', '室内湿度・空気清浄機稼働状況・換気の実施記録', '睡眠の質・気分・認知機能の日次変化を継続追跡', 'カビ環境改善の進捗（除湿・清掃・業者対応）の記録'],
      emf: ['症状発生時の場所・デバイス・電磁波関連環境を記録', '症状の強さ（頭痛・倦怠感・ブレインフォグ）を0-10で毎日記録', '電磁波の少ない環境（屋外・機内モード）での症状変化を追跡', '睡眠の質・気分・疲労スコアの日次変化の継続記録'],
    };
    const items = new Set();
