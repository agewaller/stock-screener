...
> **改訂注記 (2026-05-26)**: 本 ADR のサービス構成にある Keycloak（Cloud Run + 専用 Cloud SQL、`identity-*` プロジェクト）は撤回された。
> 認証は GCP Identity Platform (Firebase Auth、マネージドサービス) に置き換え ([ADR-0014](0014-auth-switch-to-identity-platform.md))、
> Keycloak 用の Cloud Run / Cloud SQL / `identity-*` プロジェクトは不要になった。
> Cloud Run / Cloud SQL / GCS / Secret Manager 等のコアサービス選定と環境戦略は引き続き有効。本文は当時の決定の記録として残す。

> **実装状況注記 (2026-06-01、フェーズ 6 / 2026-06-02 staging 追記)**: 本 ADR の決定は有効だが、実装は段階的:
>
> - **構築済**: 単一 GCP プロジェクト (`arctic-anvil-497002-q2`) に Cloud Run (`cares-api` / `cares-web`) + Cloud SQL + GCS + Secret Manager (フェーズ 6 最小構成)
> - **staging 環境 (2026-06-02 決定)**: 本 ADR の原設計（`cares-staging` プロジェクト分離）ではなく、
>   本番プロジェクト内に `-staging` 接尾辞リソースで共存する方式を採用（[ADR-0015](0015-staging-environment-in-prod-project.md)）。
>   組織・課金アカウントへの権限制約のため。プロジェクト分離は将来オプションとして維持。
>   構築状況は [`../OPERATIONS.md`](../OPERATIONS.md) の staging 節を参照。
> - **未実装 (フェーズ 7 以降)**: dev / staging / prod の 3 プロジェクト分離、Terraform IaC（現状は `infra/scripts/` の gcloud bash）、GitHub Actions による自動デプロイ（現状 CI はテストのみ）、Cloud DNS カスタムドメイン
>
> 現状の実構成は [`../OPERATIONS.md`](../OPERATIONS.md) §7 / [`../architecture/system-overview.md`](../architecture/system-overview.md) を参照。
