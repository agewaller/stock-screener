...
> **改訂注記 (2026-05-26)**: 本 ADR の認証移行（Firebase Auth → Keycloak、Email+Password の強制パスワードリセット、匿名ユーザーの個別対応）は前提が変わった。
> 新 cares も GCP Identity Platform (= Firebase Auth) を使う ([ADR-0014](0014-auth-switch-to-identity-platform.md)) ため、
> 認証移行は「旧 Firebase Auth → 新 Firebase Auth」となり大幅に単純化される
> （同一 Identity Platform プロジェクトに集約すれば UID 再利用で移行不要、別プロジェクト間でも Admin SDK の `importUsers` でパスワードハッシュ込み移送が可能）。
> `user_link` は廃止され `users.global_uid` 直保管。匿名認証 (FN-AUTH-03) は未実装のため移行対象なし。認証移行の詳細は要再設計。
> Firestore → PostgreSQL / Firebase Storage → Cloud Storage のデータ移行方針とカットオーバー手順は引き続き有効。本文は当時の決定の記録として残す。
