...
ER 図 [`er.md`](er.md) に対応する各テーブルの詳細定義。Prisma の `schema.prisma` を書く際の正本となる論理仕様。
...
ローカルユーザーレコード。`global_uid` (Firebase UID) を identity 主軸として直保管する（[ADR-0014](../../adr/0014-auth-switch-to-identity-platform.md)）。`email` `display_name` は Firebase ログイン時に取得。

| カラム | 型 | 制約 | 備考 |
|---|---|---|---|
| id | UUID | PK | ローカル user_id |
| global_uid | TEXT | NOT NULL, UNIQUE | Firebase UID。immutable な identity 主軸（旧設計の Keycloak `sub` 相当） |
| email | TEXT | NOT NULL, UNIQUE | Firebase から取得。Tier 2 admin 判定にも使用 |
| display_name | TEXT | | |
| is_anonymous | BOOLEAN | NOT NULL DEFAULT FALSE | 匿名ログインのフラグ（FN-AUTH-03 未実装） |
| created_at | TIMESTAMPTZ | NOT NULL DEFAULT NOW() | |
| updated_at | TIMESTAMPTZ | NOT NULL DEFAULT NOW() | |
| deleted_at | TIMESTAMPTZ | NULL | アカウント削除 |

アプリ層は ID token から `uid` を取り出し、`prisma.user.upsert({ where: { globalUid } })` でローカル user に解決する（初回ログインで自動 provision）。

### 1.2 `user_link`（廃止）

旧設計（ADR-0002）で `global_uid` と `users.id` を紐付けていたテーブル。[ADR-0014](../../adr/0014-auth-switch-to-identity-platform.md) で `users.global_uid` 直保管に変更され**廃止**。
...
| selected_diseases | TEXT[] | NOT NULL DEFAULT '{}' | DISEASE_CATEGORIES の disease id 配列 (`/api/analysis/{daily,deep,research}` で 疾患固有プロンプトを引くのに使う) |
| feature_flags | JSONB | | |
| updated_at | TIMESTAMPTZ | NOT NULL | |

注: 旧設計案では `user_profiles.selected_diseases` (JSONB) だったが、現実装ではログイン直後から使う設定値のため `user_preferences` 側に移した (`String[]` で十分軽量)。
...
実体は Cloud Storage、DB はメタのみ。FN-DIARY-02 + FN-AI-04。

| カラム | 型 | 制約 | 備考 |
|---|---|---|---|
| id | UUID | PK | |
| tenant_id | UUID | NOT NULL | MVP B2C: user_id と同値 |
| user_id | UUID | NOT NULL, FK → users | |
| gcs_object_path | TEXT | NOT NULL | アクティブ: `photos/{user_id}/{photo_id}.jpg`、ソフト削除後: `deleted/photos/...` |
| mime_type | TEXT | NOT NULL | `image/jpeg` / `image/png` / `image/webp` |
| size_bytes | INTEGER | NOT NULL | 8 MB 上限 |
| status | TEXT | NOT NULL DEFAULT 'draft' | `draft` (Signed URL 発行直後) / `ready` (アップ完了) / `failed` |
| photo_type | TEXT | | `food/blood_test/prescription/supplement/medical_doc/device/body/other` (AI 2-pass の classify 結果) |
| ai_classification | JSONB | | pass2 (`inline_image_food` 等) の構造化結果 |
| ai_model_used | TEXT | | 実際に使われたモデル ID |
| ai_analyzed_at | TIMESTAMPTZ | | 完了時刻 |
| ai_error | TEXT | | `quota_exceeded` / `quota_exceeded_after_pass1` / `classify_failed: ...` 等 |
| related_category | TEXT | | 自動関連付け先: `meals` / `medications` / `blood_tests` / `text_entries` |
| related_record_id | UUID | | 同上 record の id |
| captured_at | TIMESTAMPTZ | NOT NULL | クライアント側の撮影時刻 (現在は client now() を採用、将来 EXIF から取得) |
| created_at | TIMESTAMPTZ | NOT NULL DEFAULT NOW() | |
| updated_at | TIMESTAMPTZ | NOT NULL | |
| deleted_at | TIMESTAMPTZ | | ソフト削除。30 日後に lifecycle で物理削除 |

INDEX: `(user_id, captured_at DESC)`, `(user_id, status)`, `(related_category, related_record_id)`。

bucket / IAM / CORS / lifecycle は [`infra/gcs.md`](../../../infra/gcs.md) を参照。
...
| カラム | 型 | 制約 | 備考 |
|---|---|---|---|
| key | TEXT | PK | `universal_daily/mecfs_daily/report_doctor/inline_text_analysis/...` |
| name | TEXT | | 表示名 (例: 「ME/CFS 日次分析」) |
| description | TEXT | | |
| disease | TEXT | DEFAULT '_universal' | DISEASE_CATEGORIES の id、または `_universal` |
| schedule | TEXT | DEFAULT 'manual' | `daily/weekly/on_data_update/manual` |
| active | BOOLEAN | NOT NULL DEFAULT TRUE | 無効化フラグ |
| current_version | TEXT | | |
| updated_at | TIMESTAMPTZ | | |
| created_at | TIMESTAMPTZ | NOT NULL DEFAULT NOW() | |

INDEX: `disease`, `schedule`。

bootstrap: 起動時に `apps/api/src/legacy-prompts.json` (旧 stock-screener の 152 件) と `apps/api/src/extra-prompts.json` (cares 固有の 5 件: `report_doctor/sns/relative` + `daily_brief` + `deep_reflection`) を seed。既存行は上書きしない (admin 編集を保護)。
